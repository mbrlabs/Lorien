...
func _ready():
	...
	_pressure_sensitivity.value_changed.connect(_on_PressureSensitivity_value_changed)
	_constant_pressure.toggled.connect(_on_ConstantPressure_toggled)
	_brush_size.value_changed.connect(_on_DefaultBrushSize_value_changed)
	_tool_pressure.value_changed.connect(_on_DefaultToolPressure_value_changed)
	_project_dir.text_changed.connect(_on_DefaultSaveDir_text_changed)
	_language_options.item_selected.connect(_on_Language_OptionButton_item_selected)
	_theme.item_selected.connect(_on_Theme_item_selected)
	_ui_scale_options.item_selected.connect(_on_UIScaleOptions_item_selected)
	_canvas_color.color_changed.connect(_on_CanvasColor_color_changed)
	_grid_pattern.item_selected.connect(_on_GridPattern_item_selected)
	_grid_size.value_changed.connect(_on_GridSize_value_changed)
	_aa_mode.item_selected.connect(_on_AntiAliasing_item_selected)
	_brush_rounding_options.item_selected.connect(_on_Brush_rounding_item_selected)
	_foreground_fps.value_changed.connect(_on_Target_Fps_Foreground_changed)
	_background_fps.value_changed.connect(_on_Target_Fps_Background_changed)
